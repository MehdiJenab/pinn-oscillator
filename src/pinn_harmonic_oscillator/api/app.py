...
def create_pinn_model(input_dim=1, hidden_dim=50, output_dim=1, omega=1.0):
    ...
    """
    model = PINN(input_dim, hidden_dim, output_dim, omega=omega)
    return model


def train_model(omega, t_max, num_domain_points, hidden_dim, epochs, learning_rate, weight_decay, lambda_ic, lambda_physics):
    """Train a PINN model with given parameters."""
    print(f"Training with parameters: omega={omega}, t_max={t_max}, epochs={epochs}")
    ...
    # Create model
    pinn = create_pinn_model(input_dim=1, hidden_dim=hidden_dim, output_dim=1, omega=omega)
    ...
    for epoch in range(epochs):
        ...
        ode_residual = d2x_dt2 + omega**2 * x_domain
    ...
    residual = d2x_dt2 + omega**2 * x_pred_np
